...
const accountIndex =
  new URLSearchParams(document.location.search).get("account") ?? 1;
console.log("Using account index ", accountIndex);

export const KATANA_ACCOUNT_ADDRESS = import.meta.env[
  `VITE_KATANA_ACCOUNT_${accountIndex}_ADDRESS`
];
export const KATANA_ACCOUNT_PRIVATEKEY = import.meta.env[
  `VITE_KATANA_ACCOUNT_${accountIndex}_PRIVATEKEY`
];
...
export async function setupNetwork() {
  ...
  const provider = new Providers.RPCProvider(
    WORLD_ADDRESS,
    "http://127.0.0.1:9944"
  );
  ...
  const signer = new Account(
    ...
    ec.getKeyPair(KATANA_ACCOUNT_PRIVATEKEY)
  );
  ...
  return {
    ...
    entities: async (component: string, partition: string, length: number) =>
      provider.entities(component, partition, length),
    world,
    syncWorker,
  };
}
